...
class StringMatcher:
    def _reset_cache(self): # 重置StringMatcher里的类
        self._ratio = self._distance = None
        self._opcodes = self._editops = self._matching_blocks = None

    def __init__(self, seq1='', seq2=''): # 初始化StringMatcher里的类
        self._str1, self._str2 = seq1, seq2
        self._reset_cache()

    def set_seqs(self, seq1, seq2): # 设置类_str1和_str2
        self._str1, self._str2 = seq1, seq2
        self._reset_cache()

    def set_seq1(self, seq1): # 仅设置类_str1
        self._str1 = seq1
        self._reset_cache()

    def set_seq2(self, seq2): # 仅设置类_str2
        self._str2 = seq2
        self._reset_cache()

    def get_opcodes(self): # Levenshtein opcodes : Find sequence of edit operations transforming one string to another.
    ...
    def get_editops(self): # Levenshtein editops : Find sequence of edit operations transforming one string to another.
    ...
    def get_matching_blocks(self): # Levenshtein matching_blocks : 
        if not self._matching_blocks:
            self._matching_blocks = matching_blocks(self.get_opcodes(),
                                                    self._str1, self._str2)
        return self._matching_blocks

    def ratio(self):  # Levenshtein ratio : Compute similarity of two strings.
        if not self._ratio:
...
def normalize(name, branch_words, re_chars):
    if name:
        ...
        # jieba采用n-gram模型(不采用HMM模型)最大概率分词
...
def get_main_sub(string_array, weight_sort_amount):
    if string_array and len(string_array) >= weight_sort_amount:
        ...
        weights = [weight.get(x, 0) for x in string_array] # 'weigth' is not defined
